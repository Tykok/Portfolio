...
describe('SOUNDS', () => {
  it('gives every named sound at least one audible tone to fall back on', () => {
  ...
  it('points every sound at a sample file, which the visitor may or may not have', () => {
    for (const name of SOUND_NAMES) {
      expect(SOUNDS[name].file, name).toMatch(/^\/sounds\/[a-z]+\.wav$/);
    }
  });

  it('keeps interface feedback short enough not to overlap a fast clicker', () => {
    const end = (name: 'click' | 'menu') => Math.max(...SOUNDS[name].tones.map((t) => t.at + t.dur));
